  ...
  std::vector<std::string> listRestaurants;
    try
    {
        ...
        while( sub->isConnected() )
        {
           ...
           if (parts[0]=="?corona" && parts[1]=="1")
           {
               string land1;
               string land2;
               landen=parts[1].toStdString();
               comando=parts[3].toStdString();
               land1=parts[4].split(''');
               land2=parts[5].toStdString();
               dag=parts[6].toStdString();
               maand=parts[7].toStdString();
               ...
               if(parts[1]=="1")
                   for (vector<string> record : data)
                    {
                       if(parts[3]=="2" && record[LAND]==parts[4].toStdString() && record[DAY]==parts[6].toStdString() && record[MONTH]==parts[7].toStdString() )
               {
               naam=parts[2].toStdString();
               cout << naam << " heeft je service net gebruikt!"<< endl;
               nzmqt::ZMQMessage msg = nzmqt::ZMQMessage(QString::fromStdString("!corona>" + naam + ">" + "Hallo " + naam + ", aantal doden "  + " van " + land1 + " op " + dag + "/" + maand + "/" + "2020" + ": " + record[DEATHS]).toUtf8());
               send->sendMessage(msg);

               }
                       if(parts[3]=="2" && record[LAND]==parts[5].toStdString() && record[DAY]==parts[6].toStdString() && record[MONTH]==parts[7].toStdString() )
               {
               naam=parts[2].toStdString();
               cout << naam << " heeft je service net gebruikt!"<< endl;
               nzmqt::ZMQMessage msg = nzmqt::ZMQMessage(QString::fromStdString("!corona>" + naam + ">" + "Hallo " + naam + ", aantal doden "  + " van " + land2 + " op " + dag + "/" + maand + "/" + "2020" + ": " + record[DEATHS]).toUtf8());
               send->sendMessage(msg);

               }

                       if(parts[3]=="1" && record[LAND]==parts[4].toStdString() && record[DAY]==parts[6].toStdString() && record[MONTH]==parts[7].toStdString() )
               {
               naam=parts[2].toStdString();
               cout << naam << " heeft je service net gebruikt!"<< endl;
               nzmqt::ZMQMessage msg = nzmqt::ZMQMessage(QString::fromStdString("!corona>" + naam + ">" + "Hallo " + naam + ", aantal besmettingen "  + " van " + land1 + " op " + dag + "/" + maand + "/" + "2020" + ": " + record[CASES]).toUtf8());
               send->sendMessage(msg);

               }
                       if(parts[3]=="1" && record[LAND]==parts[5].toStdString() && record[DAY]==parts[6].toStdString() && record[MONTH]==parts[7].toStdString() )
               {
               naam=parts[2].toStdString();
               cout << naam << " heeft je service net gebruikt!"<< endl;
               nzmqt::ZMQMessage msg = nzmqt::ZMQMessage(QString::fromStdString("!corona>" + naam + ">" + "Hallo " + naam + ", aantal besmettingen "  + " van " + land2 + " op " + dag + "/" + maand + "/" + "2020" + ": " + record[CASES]).toUtf8());
               send->sendMessage(msg);
            ...
            for (vector<string> record : data)
                {
                   ...
                   else if(parts[4].toStdString() != "Afghanistan" &&
                           parts[4].toStdString() != "Albania" &&
                           parts[4].toStdString() != "Algeria" &&
                   ...
                   else if((parts[3]=="1" || parts[3]=="2" || parts[3]=="3" || parts[3]=="4" || parts[3]=="5" || parts[3]=="6") && ((parts[5].toStdString())>"14" && parts[6].toStdString()>"11"))
                   {
                   naam=parts[2].toStdString();
                   cout << naam << " heeft je service net gebruikt!"<< endl;
                   nzmqt::ZMQMessage msg = nzmqt::ZMQMessage(QString::fromStdString("!corona>" + naam + ">" + "Hallo " + naam + ", Sorry! ... De datum die je ingegeven hebt is buiten de grenzen van de database!\nProbeer eens nog een keer met een datum tussen 1/1/2020 en 14/12/2020. ").toUtf8());
                   send->sendMessage(msg);
                   break;
                   }


                   else if(parts[3]!="1" && parts[3]!="2" && parts[3]!="3" && parts[3]!="4" && parts[3]!="5" && parts[3]!="6"&& parts[3]!="7")
                   {
                   naam=parts[2].toStdString();
                   cout << naam << " heeft je service net gebruikt!"<< endl;
                   ...
                   break;
                    ...
                    else if(parts[3]=="1" && record[LAND]==parts[4].toStdString() && record[DAY]==parts[5].toStdString() && record[MONTH]==parts[6].toStdString() )
                    {
                    naam=parts[2].toStdString();
                    cout << naam << " heeft je service net gebruikt!"<< endl;
                   ...
                   else if(parts[3]=="2" && record[LAND]==parts[5].toStdString() && record[DAY]==parts[6].toStdString() && record[MONTH]==parts[7].toStdString() )
                    {
                    naam=parts[2].toStdString();
                    cout << naam << " heeft je service net gebruikt!"<< endl;
                    nzmqt::ZMQMessage msg = nzmqt::ZMQMessage(QString::fromStdString("!corona>" + naam + ">" + "Hallo " + naam + ", aantal besmettingen "  + " van " + land2 + " op " + dag + "/" + maand + "/" + "2020" + ": " + record[CASES]).toUtf8());
                ...
                for (vector<string> record : data)
                    {
                       ...
                       else if((parts[2]=="1" || parts[2]=="2" || parts[2]=="3" || parts[2]=="4" || parts[2]=="5" || parts[2]=="6") && ((parts[4].toStdString())>"14" && parts[5].toStdString()>"11"))
                       {
                       naam=parts[1].toStdString();
                       cout << naam << " heeft je service net gebruikt!"<< endl;
                       nzmqt::ZMQMessage msg = nzmqt::ZMQMessage(QString::fromStdString("!corona>" + naam + ">" + "Hallo " + naam + ", Sorry! ... De datum die je ingegeven hebt is buiten de grenzen van de database!\nProbeer eens nog een keer met een datum tussen 1/1/2020 en 14/12/2020. ").toUtf8());
